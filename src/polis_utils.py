...
def pubkey_to_address(pub_key, polis_network: str):
    """Convert public key to a Polis address."""
...
def wif_privkey_to_address(privkey: str, polis_network: str):
    pubkey = wif_privkey_to_pubkey(privkey)
    return pubkey_to_address(pubkey, polis_network)
...
def validate_address(address: str, polis_network: typing.Optional[str]) -> bool:
    ...
    try:
        ...
        if len(data) > 5:
            prefix = data[0]
            if polis_network:
                prefix_valid = (prefix == get_chain_params(polis_network).PREFIX_PUBKEY_ADDRESS or
                                prefix == get_chain_params(polis_network).PREFIX_SCRIPT_ADDRESS)
            else:
...
def generate_wif_privkey(polis_network: str, compressed: bool = False):
    """
...
def validate_wif_privkey(privkey: str, polis_network: str):
    try:
        data = base58.b58decode(privkey)
        if len(data) not in (37, 38):
            raise Exception('Invalid private key length')

        if data[0] != get_chain_params(polis_network).PREFIX_SECRET_KEY:
...
def wif_to_privkey(wif_key: str, polis_network: str):
    """
    Based on project: https://github.com/chaeplin/polismnb with some changes related to usage of bitcoin library.
    """
    privkey_encoded = base58.b58decode(wif_key).hex()
    wif_prefix_cur = privkey_encoded[:2]
    wif_prefix_network = get_chain_params(polis_network).PREFIX_SECRET_KEY
...
def ecdsa_sign_raw(msg_raw: bytes, wif_priv_key: str, polis_network: str):
...
class CMasternodePing(object):
    ...
    def sign_message(self, priv_key, polis_network):
        ...
        r = ecdsa_sign(self.sig_message, priv_key, polis_network)
        self.sig = base64.b64decode(r)
        return self.sig

    def sign(self, priv_key, polis_network, is_spork6_active: bool):
        if is_spork6_active:
            hash = self.get_hash()
            r = ecdsa_sign_raw(hash, priv_key, polis_network)
            self.sig = base64.b64decode(r)
        else:
            self.sig = self.sign_message(priv_key, polis_network)
        return self.sig
...
class CMasternodeBroadcast(object):
    ...
    def sign(self, collateral_bip32_path: str, hw_sign_message_fun: typing.Callable, hw_session,
        ...
        self.mn_ping.sign(mn_privkey_wif, polis_network, is_spork6_active=self.spork6_active)
